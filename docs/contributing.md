...
description: Git/CLI-процесс добавления, перевода и удаления байткодов.
---

# Как добавить байткод
...
Slug отвечает на вопрос «что делает», а не «который по счёту». Если хочется назвать `foo-2` — это сигнал, что либо различие не сформулировано (сформулируйте: `foo-batch`, `foo-async`), либо это правка старого байткода (отредактируйте его).
